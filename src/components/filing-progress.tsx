  ...
  if (halted) {
    return (
      <div className="rounded-lg border border-destructive/40 bg-destructive/10 p-4">
        <div className="text-sm font-semibold text-destructive">Filing Halted</div>
        {haltReason && <div className="mt-1 text-xs text-destructive/80">{haltReason}</div>}
      </div>
    );
  }
  ...
  return (
    <ol className="flex w-full items-center gap-1 overflow-x-auto">
      {STEPS.map((step, i) => {
        ...
        const active = i === idx;
        return (
          <li key={step} className="flex flex-1 min-w-[90px] items-center gap-1">
